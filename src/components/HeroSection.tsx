...
const HeroSection: React.FC = () => {
  const { onlineUsers, chatMessages, newsUpdates } = useRealtime();
  return (
    <>
      <Navbar />
      <div className="relative bg-gradient-to-r from-yellow-400 via-blue-500 to-red-500 min-h-screen flex items-center justify-center pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          <div className="bg-white bg-opacity-30 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white border-opacity-30">
            ...
            {/* Real-time stats */}
            <div className="mb-8 flex justify-center items-center space-x-6 text-sm text-gray-700">
              <div className="flex items-center">
                <span className="w-2 h-2 bg-green-500 rounded-full mr-2"></span>
                {onlineUsers} usuarios conectados
              </div>
              <div>📨 {chatMessages.length} mensajes hoy</div>
              <div>📰 {newsUpdates.length} noticias actualizadas</div>
            </div>
